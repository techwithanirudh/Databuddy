 ...
 */
export function createPerformanceTimeSeriesBuilder(websiteId: string, startDate: string, endDate: string) {
  const builder = createSqlBuilder();
 ...
 */
export function createPerformanceByDeviceBuilder(websiteId: string, startDate: string, endDate: string) {
  const builder = createSqlBuilder();
  builder.setTable('events');
  ...
  builder.sb.where = {
    ...
    metric_filter: "(load_time IS NOT NULL OR ttfb IS NOT NULL OR fcp IS NOT NULL OR lcp IS NOT NULL)"
  };
