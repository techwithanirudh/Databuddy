...
- events table columns: ${JSON.stringify(AnalyticsSchema.columns.map(col => ({ name: col.name, type: col.type })), null, 2)};
- There is also an 'analytics.errors' table for error analysis.

RESPONSE FORMAT (JSON - ONLY THIS, NO EXTRA TEXT OR MARKDOWN):
{
  "thinking_steps": ["Brief step-by-step plan to answer the query..."],
...
- Do NOT just say "I can't answer". INSTEAD, suggest a few alternative, specific questions based on the available schema.
- Example for a vague query like "show me performance":
{
  "response_type": "text",
  "sql": null,
  "chart_type": null,
