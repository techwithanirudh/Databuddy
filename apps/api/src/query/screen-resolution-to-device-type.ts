 ...
 * @returns DeviceType
 */
export function mapScreenResolutionToDeviceType(screenResolution: string): DeviceType {
    if (!screenResolution || typeof screenResolution !== 'string') return 'unknown';
    const parts = screenResolution.split('x');
    if (parts.length !== 2 || typeof parts[0] !== 'string' || typeof parts[1] !== 'string') return 'unknown';
    ...
    if (width > 800 && width <= 1280 && aspect >= 1.1 && aspect <= 1.7) return 'tablet';

    // Laptops: width up to 1920, aspect ratio typical for laptops
    if (width > 1280 && width <= 1920 && aspect >= 1.3 && aspect <= 1.8) return 'laptop';
    ...
    if (width > 1920 && width <= 2560 && aspect >= 1.3 && aspect <= 2.0) return 'desktop';
