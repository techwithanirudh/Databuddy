    ...
    return (
        <DropdownMenu onOpenChange={setIsOpen} open={isOpen}>
            <DropdownMenuTrigger asChild>
                <Button
                    className={cn(
                        "h-9 justify-start gap-2 rounded border border-border/50 bg-background px-3 text-sm font-normal hover:bg-accent hover:text-accent-foreground",
                        className
                    )}
                    variant="outline"
                >
                    <Avatar className="h-5 w-5">
                        ...
                        <AvatarFallback className="bg-accent text-xs">
                            {activeOrganization ? getOrganizationInitials(activeOrganization.name) : "P"}
                        </AvatarFallback>
                    </Avatar>
                ...
                >
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-2">
                            ...
                            {!activeOrganization && <CheckIcon className="h-3 w-3 text-primary" size={12} />}
                        </div>
                        <p className="text-muted-foreground text-xs">Your personal workspace</p>
                    </div>
                </DropdownMenuItem>
