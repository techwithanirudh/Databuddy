...
export const groupBrowserData = (
  ...
  return Object.entries(browserCounts as Record<string, { visitors: number }>).map(
    ([browser, data]) => ({
      name: browser,
      value: data.visitors,
    })
...
export const formatDomainLink = (
  ...
  const displayPath = path.length > maxLength ? `${path.slice(0, maxLength - 3)}...` : path;
...
export function isTrackingNotSetup(analytics: any): boolean {
  ...
  const hasEvents = events_by_date?.some(
    (event: any) => (event.pageviews || 0) > 0 || (event.visitors || event.unique_visitors || 0) > 0
  );
