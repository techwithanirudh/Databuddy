import { BugIcon, MonitorIcon, PhoneIcon, TableIcon } from "@phosphor-icons/react";
  ...
  cell: (info: any) => {
      ...
      return (
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <Badge className="border-gray-200 bg-gray-100 text-gray-800">Unknown Error</Badge>
          </div>
          <p className="text-muted-foreground text-sm">No error message available</p>
        </div>
      );
    }
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        <p className="line-clamp-2 max-w-md text-muted-foreground text-sm" title={message}>
          {message}
        </p>
      </div>
    );
...
export const createDeviceColumn = () =>
  createNameColumn("Device Type", (name) => {
    if (!name) return <MonitorIcon className="h-4 w-4 text-gray-500" size={16} weight="duotone" />;
    ...
    return device.includes("mobile") || device.includes("phone") ? (
      ...
      <TableIcon className="h-4 w-4 text-purple-500" size={16} weight="duotone" />
    ) : (
      <MonitorIcon className="h-4 w-4 text-gray-500" size={16} weight="duotone" />
    );
  });
