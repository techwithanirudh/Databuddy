        ...
        if (!message) {
            return (
                <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                        <BugIcon size={16} weight="duotone" className="h-4 w-4" />
...
export const createDeviceColumn = () =>
    createNameColumn('Device Type', (name) => {
        if (!name) return <MonitorIcon size={16} weight="duotone" className="h-4 w-4 text-gray-500" />;

        const device = name.toLowerCase();
        return device.includes('mobile') || device.includes('phone') ?
            <PhoneIcon size={16} weight="duotone" className="h-4 w-4 text-blue-500" /> :
            device.includes('tablet') ?
                <TableIcon size={16} weight="duotone" className="h-4 w-4 text-purple-500" /> :
                <MonitorIcon size={16} weight="duotone" className="h-4 w-4 text-gray-500" />;
    });
