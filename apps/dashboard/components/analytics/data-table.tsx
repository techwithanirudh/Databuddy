...
import { type ColumnDef, type RowData, flexRender, getCoreRowModel, useReactTable, getFilteredRowModel, getSortedRowModel, type SortingState, getExpandedRowModel, type ExpandedState } from "@tanstack/react-table";
...
export function DataTable<TData extends { name: string | number }, TValue>(
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    estimateSize: (index) => {
      ...
      if (row.getIsExpanded() && renderSubRow) {
        // A rough estimation for expanded custom rows
        return 150;
      }
              ...
              <Table>
                ...
                <TableBody style={{ height: `${totalSize}px`, position: 'relative' }}>
                  ...
                  {virtualRows.map(virtualRow => {
                    ...
                    // Special rendering for custom sub-rows
                    if (row.depth > 0 && renderSubRow && parentRow) {
                      return (
                        <TableRow
                          key={row.id}
                        ...
                        >
                          <TableCell colSpan={table.getAllColumns().length} className="p-0">
                            {renderSubRow(row.original as TData, parentRow.original as TData, row.index)}
                          </TableCell>
                        </TableRow>
                      )
                    }
                    ...
                    return (
                      <TableRow
                        ...
                        ref={node => rowVirtualizer.measureElement(node)}
                        className={cn(
                          "border-border/20 transition-all duration-300 ease-in-out absolute w-full",
                          (onRowClick && !row.getCanExpand()) || row.getCanExpand() ? "cursor-pointer" : "",
                          hoveredRow && hoveredRow !== row.id ? "opacity-40 grayscale-[80%]" : "opacity-100",
                          !hoveredRow && (virtualRow.index % 2 === 0 ? "bg-background/50" : "bg-muted/10")
                        )}
                        onClick={() => {
                          if (row.getCanExpand()) {
                            row.toggleExpanded();
                        ...
                        <div className="flex w-full">
                          {row.getVisibleCells().map((cell, cellIndex) => (
                            <TableCell
                              key={cell.id}
                              className={cn(
                                "py-3 text-sm font-medium transition-colors duration-150 px-2 sm:px-4 flex items-center",
                              ...
                              style={{
                                ...
                                minWidth: `calc(${cell.column.getSize()}px / 2)`,
                                paddingLeft: cellIndex === 0 ? `${row.depth * 1.5 + 1}rem` : undefined
                              }}
                            >
...
function CellTooltip<TData extends { name: string | number }, TValue>({ cell }: { cell: any }) {
  const cellContent = flexRender(cell.column.columnDef.cell, cell.getContext());
  const cellValue = cell.getValue();

  const isSimpleContent = typeof cellContent === 'string' || typeof cellContent === 'number';
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="truncate block">{cellContent}</div>
        </TooltipTrigger>
        {cellValue && (
          <TooltipContent>{String(cellValue)}</TooltipContent>
        )}
      </Tooltip>
    </TooltipProvider>
  );
