...
export function DataTable<TData extends { name: string | number }, TValue>(
  ...
  const [pagination, setPagination] = React.useState<PaginationState>({
    pageIndex: 0,
    pageSize: isScrollable ? (data?.length ?? 10) : (initialPageSize ?? 10),
  });
  ...
  const table = useReactTable({
    ...
    onPaginationChange: isScrollable ? undefined : setPagination,
  ...
  return (
    <Card className={cn("w-full border shadow-sm bg-card/50 backdrop-blur-sm overflow-hidden", className)}>
      ...
      <CardContent className="p-0" style={{ minHeight }}>
        <AnimatePresence mode="wait">
          ...
          >
            <div
              className={cn("overflow-hidden", isScrollable && "overflow-y-auto rounded-lg border")}
              style={isScrollable ? { maxHeight: scrollHeight } : {}}
            >
              <Table className={cn(isScrollable && "h-full")}>
      ...
      {!isScrollable && tableData.length > (pagination.pageSize || 10) && (
