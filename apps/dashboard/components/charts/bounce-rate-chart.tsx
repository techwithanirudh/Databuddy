...
export function BounceRateChart({ data, isLoading = false }: BounceRateChartProps) {
    ...
    <Card className="bg-slate-900/80 border-slate-800/80 backdrop-blur-sm overflow-hidden transition-all hover:shadow-md hover:shadow-sky-500/5 hover:border-sky-500/30">
      <CardHeader className="pb-2">
        ...
        <div className="flex items-baseline gap-2 mt-2">
          <p className="text-2xl font-bold text-white">{latestValue}%</p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={formattedData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              ...
              <Line 
                ...
                strokeWidth={2}
                dot={{ fill: '#fbbf24', r: 4 }}
                activeDot={{ fill: '#fbbf24', r: 6, strokeWidth: 2 }}
              />
            </LineChart>
          </ResponsiveContainer>
        )}
