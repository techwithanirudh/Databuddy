...
export function useCreateWebsite() {
	...
	return trpc.websites.create.useMutation({
		onMutate: async (newWebsite) => {
			const queryKey = { organizationId: newWebsite.organizationId ?? undefined };
...
export function useUpdateWebsite() {
	...
	return trpc.websites.update.useMutation({
		onMutate: async (updatedWebsite) => {
			...
			const listKey = { organizationId: previousWebsite?.organizationId ?? undefined };
		...
		onError: (err, updatedWebsite, context) => {
			...
			if (context?.previousWebsite) {
				utils.websites.getById.setData({ id: updatedWebsite.id }, context.previousWebsite);
			}
		},
		onSettled: (data, error, variables, context) => {
...
export function useDeleteWebsite() {
	...
	return trpc.websites.delete.useMutation({
		onMutate: async ({ id }) => {
			...
			utils.websites.list.setData(listKey, (old) => old?.filter((w) => w.id !== id) ?? []);
