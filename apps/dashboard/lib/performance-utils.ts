...
export function calculatePerformanceSummary(pages: PerformanceEntry[]): PerformanceSummary {
    ...
    for (const page of pages) {
      ...
      if (loadTime <= SCORE_THRESHOLDS.FAST) {
        ...
        score = 50 - ((loadTime - SCORE_THRESHOLDS.MODERATE) / SCORE_THRESHOLDS.SLOW_PENALTY_RANGE) * 50;
      }
    ...
    const performanceScore = totalVisitorWeight > 0 ? Math.round(weightedScoreSum / totalVisitorWeight) : 0;
