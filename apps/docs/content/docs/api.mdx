...
<CodeBlock
  language="json"
  code={`{
  "query": "SELECT properties.browser_name, properties.user_id:int, count() as events FROM analytics.events WHERE properties.browser_name IS NOT NULL GROUP BY properties.browser_name, properties.user_id:int ORDER BY events DESC LIMIT 10",
  "clientId": "your-client-id"
}`}
/>
...
```sql
-- Instead of writing:
SELECT JSONExtractString(properties, 'browser_name') FROM analytics.events

-- You can simply write:
SELECT properties.browser_name FROM analytics.events
```
