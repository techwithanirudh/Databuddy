...
export function generateSitemapEntries(): MetadataRoute.Sitemap {
  ...
  try {
    ...
    for (const page of pages) {
      const url = page.url
      const priority = priorityMap[url] || (url.includes('/Integrations/') ? 0.7 : 0.6)
    ...
    const fallbackEntries = [
      ...
      { url: '/docs/domain-verification', priority: 0.8, changeFrequency: 'monthly' as const },
      { url: '/docs/dashboard', priority: 0.8, changeFrequency: 'weekly' as const },
      ...
      { url: '/docs/Integrations', priority: 0.8, changeFrequency: 'weekly' as const },
      { url: '/docs/Integrations/react', priority: 0.8, changeFrequency: 'weekly' as const },
...
export function getSitemapMetadata() {
  return {
    ...
    description: 'Dynamically generated sitemap of Databuddy documentation including all guides, integrations, and API references.',
    keywords: ['databuddy', 'analytics', 'documentation', 'sitemap', 'privacy-first', 'web analytics'],
  }
} 