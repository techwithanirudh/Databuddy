...
describe('drizzle-cache', () => {
    it('caches and returns values (hit/miss)', async () => {
        let calls = 0;
        const result = await cache.withCache({ key: 'a', queryFn: async () => { calls++; return 'foo'; } });
        expect(result).toBe('foo');
        expect(calls).toBe(1);
        const hit = await cache.withCache({ key: 'a', queryFn: async () => { calls++; return 'bar'; } });
        expect(hit).toBe('foo');
        expect(calls).toBe(1);
    });
    ...
    it('expires cache after TTL', async () => {
        let calls = 0;
        await cache.withCache({ key: 'b', ttl: 1, queryFn: async () => { calls++; return 'baz'; } });
        await new Promise(r => setTimeout(r, 1100));
        const result = await cache.withCache({ key: 'b', ttl: 1, queryFn: async () => { calls++; return 'qux'; } });
    ...
    it('single-flight: only one queryFn runs on concurrent miss', async () => {
        let calls = 0;
        const fn = async () => { calls++; await new Promise(r => setTimeout(r, 10)); return 'sf'; };
    ...
    it('granular invalidation by table', async () => {
        await cache.withCache({ key: 'c', tables: ['t1'], queryFn: async () => 'v1' });
        await cache.withCache({ key: 'd', tables: ['t1'], queryFn: async () => 'v2' });
    ...
    it('partial invalidation by key', async () => {
        await cache.withCache({ key: 'g', tables: ['t2'], queryFn: async () => 'v1' });
        await cache.withCache({ key: 'h', tables: ['t2'], queryFn: async () => 'v2' });
    ...
    it('cleanupDeps removes empty dep/tag sets', async () => {
        await cache.withCache({ key: 'i', tables: ['t3'], tag: 'tag3', queryFn: async () => 'v' });
