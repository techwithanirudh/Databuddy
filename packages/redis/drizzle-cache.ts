...
export function createDrizzleCache({ redis, namespace = 'cache' }: CacheConfig) {
        ...
        async withCache<T>({
            ...
            try {
                ...
                if (existing) {
                    ...
                    logIfDebug('info', `[drizzle-cache] HIT: ${cacheKey} (${duration}ms)`)
                    return JSON.parse(existing)
                }
            } catch (err) {
                logIfDebug('error', `[drizzle-cache] ERROR: Redis GET failed for ${cacheKey}`, err)
            }
            ...
            const missPromise = (async () => {
                ...
                try {
                    ...
                    logIfDebug('info', `[drizzle-cache] SET+EXPIRE: ${cacheKey} (${setDuration}ms, ttl=${ttl}s)`)
                } catch (err) {
                    logIfDebug('error', `[drizzle-cache] ERROR: Redis SET/EXPIRE failed for ${cacheKey}`, err)
                }
                ...
                if (autoInvalidate) {
                    ...
                    try {
                        ...
                        logIfDebug('info', `[drizzle-cache] AUTO-INVALIDATE: tables=[${tables.join(',')}] tag=${tag ?? ''} (${invDuration}ms)`)
                    } catch (err) {
                        logIfDebug('error', `[drizzle-cache] ERROR: Auto-invalidate failed for ${cacheKey}`, err)
                    }
                }

                const duration = Date.now() - missStart
                logIfDebug('info', `[drizzle-cache] MISS-RESOLVED: ${cacheKey} (${duration}ms)`)
